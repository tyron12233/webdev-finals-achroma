...
export default function SceneCanvas({ isTouch, flashOn }: { isTouch: boolean; flashOn: boolean }) {
  return (
    <Canvas
      id="r3f-canvas"
      shadows={!isTouch}
    ...
    >
      ...
      <Suspense fallback={null}>
        <Physics gravity={[0, -9.81, 0]}>
          <Environment preset="night" background={false} environmentIntensity={0.7} />

          <HorrorCorridor position={[2, -0.005, 2]} rotation={[0, 0, 0]} scale={[0.35, 0.35, 0.35]} />
          ...
          {!isTouch && (
            <ContactShadows position={[0, -0.49, 0]} opacity={0.4} scale={30} blur={3} far={15} />
          )}

          <FPSControls speed={0.8} eyeHeight={1.85} capsuleHeight={1.0} capsuleRadius={0.3} />
